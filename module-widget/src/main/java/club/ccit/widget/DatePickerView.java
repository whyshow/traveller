...
/**
 * FileName: DatePickerView
 *
 * @author: 张帅威
 * Date: 2022/2/10 8:27 上午
 * Description: 日期选择器
 * Version:
 */
public class DatePickerView extends View {
    ...
    private static class ScrollHandler extends Handler {
        private WeakReference<DatePickerView> mWeakView;

        private ScrollHandler(DatePickerView view) {
            mWeakView = new WeakReference<>(view);
        }

        @Override
        public void handleMessage(Message msg) {
            DatePickerView view = mWeakView.get();
            if (view == null){
                return;
            }
            view.keepScrolling();
